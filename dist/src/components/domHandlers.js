...
function updateContactInDOM(prevContact, contact) {
    ...
    const { contactList: prevContactList, itemCount: prevItemCount } = getContactListAndItemCount(prevLetter);
...
function updateContactItem(contactItem, contact) {
    ...
    editButton.parentNode.replaceChild(newEditButton, editButton);
    contactItem.querySelector('.contact__name').textContent = `Имя: ${contact.name}`;
    contactItem.querySelector('.contact__vacancy').textContent = `Должность: ${contact.vacancy}`;
    contactItem.querySelector('.contact__phone').textContent = `Телефон: ${contact.phone}`;
...
function renderContactsInModal(contacts = []) {
    ...
    contacts.forEach((contact) => {
        ...
        const contactItem = contactClone.querySelector('.contact');
        const editButton = contactItem.querySelector('.contact__edit-btn');
        updateContactItem(contactItem, contact);
        const deleteButton = contactItem.querySelector('.contact__delete-btn');
        deleteButton.addEventListener('click', () => {
            const letter = contact.name[0].toLowerCase();
            const contactList = document.querySelector(`[data-id="${letter}"] .contact__list`);
            const itemCount = document.querySelector(`[data-id="${letter}"] .item__button-count`);
...
function renderColumn(column, letters) {
    ...
    for (let letter of letters) {
        ...
        const item = cloneItem.querySelector('.table-item');
...
function renderContactList(contactList, itemCount, letter) {
    ...
    contactList.innerHTML = '';
    contacts.forEach((contact) => {
        const cloneContactItem = contactTemplate.content.cloneNode(true);
        const contactItem = cloneContactItem.querySelector('.contact');
        updateContactItem(contactItem, contact);
        contactItem
...
//# sourceMappingURL=domHandlers.js.map